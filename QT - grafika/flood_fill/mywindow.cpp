...
void MyWindow::fill(int r, int g, int b){
    ...
    while(!stos.empty()){
        punkt = stos.top();
        stos.pop();
        ar = ptr[szer*4*punkt.second + 4*punkt.first + 2];
        ag = ptr[szer*4*punkt.second + 4*punkt.first + 1];
        ab = ptr[szer*4*punkt.second + 4*punkt.first];

        if(r == ar && b == ab && g == ag){
            w = e = punkt.first;
            ar = ptr[szer*4*punkt.second  + 4*w+ 2];
            ag = ptr[szer*4*punkt.second  + 4*w + 1];
            ab = ptr[szer*4*punkt.second  + 4*w];
            while(r == ar && b == ab && g == ag && w >= 0){
                w--;
                ar = ptr[szer*4*punkt.second  + 4*w + 2];
                ag = ptr[szer*4*punkt.second  + 4*w + 1];
                ab = ptr[szer*4*punkt.second  + 4*w];
            }
            ar = ptr[szer*4*punkt.second  + 4*e + 2];
            ag = ptr[szer*4*punkt.second  + 4*e + 1];
            ab = ptr[szer*4*punkt.second  + 4*e];
            while(r == ar && b == ab && g == ag && e < szer){
                e++;
                ar = ptr[szer*4*punkt.second  + 4*e + 2];
                ag = ptr[szer*4*punkt.second  + 4*e + 1];
                ab = ptr[szer*4*punkt.second  + 4*e];
            ...
            for(int i = w + 1; i < e; i++){
                ...
                if(punkt.second + 1 < wys){
                    ar = ptr[szer*4*(punkt.second+1)  + 4*i + 2];
                    ag = ptr[szer*4*(punkt.second+1)  + 4*i + 1];
                    ab = ptr[szer*4*(punkt.second+1)  + 4*i ];
                    if(r == ar && b == ab && g == ag){
