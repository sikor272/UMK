    ...
    *img = copy->copy();
    ...
    if(abs(endX - clickX) > abs(endY - clickY)){
        ...
        for(int i = clickX;i >= 0 && i <= endX && i < szer; i++){
            ...
            w = a * i + b;
            pom = round(w);
            ptr[szer*4*pom + 4*i] = kolor;
            ptr[szer*4*pom + 4*i + 1] = kolor;
            ptr[szer*4*pom + 4*i + 2] = kolor;
        }
    } else {
        if(clickY > endY){
            pom = clickX;
        ...
        for(int i = clickY; i >= 0 && i <= endY && i < wys; i++){
            ...
            ptr[szer*4*i + 4*pom] = kolor;
            ptr[szer*4*i + 4*pom + 1] = kolor;
            ptr[szer*4*i + 4*pom + 2] = kolor;
        }
    }
    clickX = useX;
    clickY = useY;
